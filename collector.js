...
(function() {
  var BaseModel, Bindable, BindableSchema, Schema, async, database, decorateError, mongoObjectID, oidTest, _, _convertOidFields, _sanitizeDocuments, _validateAllFields, _validateSomeFields;
  ...
  database = require("../util/database").init("default");

  Schema = require('mongo-json-schema');
  ...
  BaseModel = (function() {
    ...
    BaseModel.prototype.outputFormatter = function(f) {
      return this.__outputFormatter = f;
    };
    ...
    BaseModel.prototype.schema = function(schema, options) {
      if (!schema) {
        ...
        return this.jsonSchema = Schema(schema, options);
      }
    };
    ...
    BaseModel.prototype.indices = indices = function(indices) {
      return this.db((function(_this) {
        return function(err, db) {
          var entry, index, options, _i, _len, _results;
          if (err != null) {
            console.flog("Error (1) insuring index for " + _this.__collectionName, err);
          }
          _results = [];
          for (_i = 0, _len = indices.length; _i < _len; _i++) {
            ...
            _results.push(db.ensureIndex(_this.__collectionName, index, options, function(err) {
              if (err) {
                return console.flog("Error (2) insuring index for " + _this.__collectionName, err);
              }
            }));
          }
          return _results;
    ...
    BaseModel.prototype.db = function(cb) {
      var db;
      db = this.overrideDatabase || database;
      return db.get(cb);
    };
    ...
    BaseModel.prototype.collection = function(cb) {
      return this.db((function(_this) {
        return function(err, db) {
          if (err || !db) {
            console.flog("Error fetching database for " + _this.__collectionName);
            return cb(err || new Error("error fetching database"));
          }
          return cb(null, db.collection(_this.__collectionName));
        };
      })(this));
    };
    ...
    BaseModel.prototype.isObjectID = function(str) {
      ...
      try {
        ...
        ex = _error;
        if ((ex != null ? ex.message : void 0) === 'Argument passed in must be a single String of 12 bytes or a string of 24 hex characters') {
          return false;
        }
        if ((ex != null ? ex.message : void 0) === 'Value passed in is not a valid 24 character hex string') {
          return false;
        }
        console.log("ex: ", ex);
      ...
      return _convertOidFields(this, query, (function(_this) {
        return function(err) {
          ...
          return _this.collection(function(err, collection) {
            ...
            if (this.__collectionName === 'custombehaviors') {
              console.log("REMOVING CUSTOMBEHAVIOR WITH QUERY:");
              console.log(query);
              if ((arguments != null ? (_ref = arguments.callee) != null ? (_ref1 = _ref.caller) != null ? _ref1.name : void 0 : void 0 : void 0) != null) {
    ...
    for (_i = 0, _len = objects.length; _i < _len; _i++) {
      ...
      if (me.__oidFields) {
        ...
        for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
          field = _ref[_j];
          try {
            if ((object[field] != null) && !(object[field] instanceof mongoObjectID) && !(_.isObject(object[field]))) {
